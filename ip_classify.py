...
def is_ipaddr(ipaddr):
    ipaddr_list = re.split("[./]",ipaddr)
    if len(ipaddr_list) < 4 and len(ipaddr_list) > 5:
        return False
...
def is_nwaddr(ipaddr):
    if not is_ipaddr(ipaddr):
        return False
    ipaddr_list = re.split("[./]",ipaddr)
    if len(ipaddr_list) != 5:
        return "Please Specify the Subnet Mask"
    mask = int(ipaddr_list[4])
    if mask == 32:
        return False
    ipaddr_list = ipaddr_list[:4]
    b = ""
    for octet in ipaddr_list:
        b = b + str(format(int(octet),'b').zfill(8))
    host = b[mask:]
    nwmask = "0" * (32 - mask)
    if host != nwmask:
        return False
    if int(b[24:],2) > int(nwmask.zfill(8),2):
...
def is_broadcast(ipaddr):
    ...
    if host != brmask:
        return False
    if int(b[24:],2) > int(brmask.zfill(8),2):
        return "Out of IP address Range"
